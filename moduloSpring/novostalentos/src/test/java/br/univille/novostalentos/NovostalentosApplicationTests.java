...
class NovostalentosApplicationTests {
	...
	void verificaClienteRetorno() throws Exception {
		MvcResult result = mockMvc.perform(get("/api/v1/clientes"))
			.andExpect(status().isOk())
			.andReturn();
	...
	@Test
	void verificaClientePeloNomee() throws Exception {
		MvcResult result = mockMvc.perform(get("/api/v1/clientes/nome/ze"))
